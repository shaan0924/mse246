...
raw_data_small <- 
  ...
  select(GrossApproval, ApprovalFiscalYear, ApprovalDate, terminus, term_years, GrossChargeOffAmount, ProjectState, BorrState, TermInMonths)
...
raw_data_tiny %>% 
  ...
  mutate(
    BinaryIntergerTerm = case_when(TermInMonths %% 12 == 0 ~ 1, TRUE ~ 0),
    BinaryThirdPartyDollars = 
      case_when(ThirdPartyDollars ~ 1, !ThirdPartyDollars ~ 0, TRUE ~ as.double(NA)),
    BinaryBankStEqualBorrowerSt = 
      case_when(
        as.character(BorrState) == as.character(CDC_State) ~ 1, 
        as.character(BorrState) != as.character(CDC_State)~ 0,
        TRUE ~ NA_integer_
    ),
    BinaryProjectStEqualBorrowerSt =
      case_when(
        as.character(BorrState) == as.character(ProjectState) ~ 1, 
        as.character(BorrState) != as.character(ProjectState) ~ 0, 
        TRUE ~ NA_integer_
    ),
